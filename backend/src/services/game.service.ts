...
const calcEloChange = (playerElo: number, opponentElo: number, result: 'win' | 'draw' | 'loss'): number => {
  const score = result === 'win' ? 1 : result === 'draw' ? 0.5 : 0;
  const expected = 1 / (1 + Math.pow(10, (opponentElo - playerElo) / 400));
  const k = playerElo < (GAME_CONFIG.ELO_THRESHOLD ?? 2000) ? (GAME_CONFIG.K_FACTOR_LOW ?? 32) : (GAME_CONFIG.K_FACTOR_HIGH ?? 16);
...
interface MoveRecord {
  ...
  timestamp: number; promotion?: string; captured?: string;
}
...
export class GameService {
  ...
  async createGameSession(
    ...
    blackPlayerId: string,
    gameId = uuidv4(),
    opts?: { whiteUsername?: string; blackUsername?: string; whiteElo?: number; blackElo?: number },
  ): Promise<GameSession> {
    ...
    try {
      await prisma.game.create({ data: {
        id: gameId, whitePlayerId, blackPlayerId,
        fen: getInitialFen(), status: 'IN_PROGRESS',
        whiteTime: GAME_CONFIG.INITIAL_TIME, blackTime: GAME_CONFIG.INITIAL_TIME, moves: '[]',
      } });
    } catch (e) {
      logger.warn('DB create game failed (non-fatal for guests)', { error: (e as Error).message });
    }
    return session;
  }
  ...
  async makeMove(gameId: string, userId: string, from: string, to: string, promotion?: string) {
    ...
    if (!isWhite && userId !== session.blackPlayerId) return { success: false, error: 'Not a player' };
    const expectedTurn: 'w' | 'b' = isWhite ? 'w' : 'b';
    if (session.turn !== expectedTurn) return { success: false, error: 'Not your turn' };
    ...
    // Clock deduction
    const now = Date.now();
    const last = session.moveHistory.length > 0
    ...
    if (session.whiteTime <= 0 || session.blackTime <= 0) {
      const lost = session.whiteTime <= 0 ? 'w' : 'b';
      await this.handleTimeout(gameId, lost);
      return { success: false, error: 'Time expired' };
    }

    const record: MoveRecord = {
      from, to, san: mv.san ?? '', fen: mv.fen ?? session.fen,
      timestamp: now, promotion, captured: mv.captured,
    };
    session.fen = mv.fen!;
    ...
    // FIX: emit move_made with flat shape the frontend expects
    // Frontend's applyMove() in useSocket.ts receives a ChessMove directly
    const movePayload = {
      from, to, san: mv.san, flags: mv.flags ?? '',
      piece: expectedTurn === 'w' ? 'w' : 'b',
      captured: mv.captured,
      promotion,
    };
    // Also emit timer_sync with { white, black } which matches useSocket.ts
    ...
    if (over.over) {
      if (over.result === 'checkmate')       session.status = isWhite ? 'white_wins' : 'black_wins';
      else if (over.result === 'stalemate') session.status = 'stalemate';
  ...
  async handleGameEnd(gameId: string, session: GameSession, overrideReason?: string) {
    ...
    const wChange = calcEloChange(session.whiteElo, session.blackElo, winner === 'w' ? 'win' : winner === 'b' ? 'loss' : 'draw');
    const bChange = calcEloChange(session.blackElo, session.whiteElo, winner === 'b' ? 'win' : winner === 'w' ? 'loss' : 'draw');

    // FIX: emit game_over in the shape useSocket.ts expects: { result: { winner, reason } }
    ...
    try {
      const dbStatus =
        ...
        (session.status === 'white_resigned' ||
         session.status === 'black_resigned')? 'RESIGNED'   : 'DRAW';

      await Promise.all([
        prisma.user.update({ where: { id: session.whitePlayerId }, data: {
          gamesPlayed: { increment: 1 },
          elo: Math.max(GAME_CONFIG.MIN_ELO ?? 100, session.whiteElo + wChange),
          ...(winner === 'w' ? { wins: { increment: 1 } } : winner === 'b' ? { losses: { increment: 1 } } : { draws: { increment: 1 } }),
        } }).catch(() => {}),
        prisma.user.update({ where: { id: session.blackPlayerId }, data: {
          gamesPlayed: { increment: 1 },
          elo: Math.max(GAME_CONFIG.MIN_ELO ?? 100, session.blackElo + bChange),
          ...(winner === 'b' ? { wins: { increment: 1 } } : winner === 'w' ? { losses: { increment: 1 } } : { draws: { increment: 1 } }),
        } }).catch(() => {}),
        prisma.game.update({ where: { id: gameId }, data: {
          status: dbStatus,
          winner: winner === 'w' ? session.whitePlayerId : winner === 'b' ? session.blackPlayerId : null,
          reason, completedAt: new Date(), pgn: '',
        } }).catch(() => {}),
      ]);
    } catch (e) { logger.error('DB game end update failed', { error: (e as Error).message }); }
  }

  async resign(gameId: string, userId: string) {
    const session = gameSessions.get(gameId);
    if (!session || session.status !== 'in_progress') return { success: false };
    if (userId !== session.whitePlayerId && userId !== session.blackPlayerId) return { success: false };
    session.status = userId === session.whitePlayerId ? 'white_resigned' : 'black_resigned';
    gameSessions.set(gameId, session);
  ...
  async getGameHistory(userId: string, limit = 20, offset = 0) {
    const games = await prisma.game.findMany({
      where: { OR: [{ whitePlayerId: userId }, { blackPlayerId: userId }], status: { not: 'IN_PROGRESS' } },
      orderBy: { completedAt: 'desc' }, take: limit, skip: offset,
