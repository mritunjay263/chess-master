...
class StockfishEngine {
    ...
    started = false;
    constructor(pathOrFactory) {
        this.pathOrFactory = pathOrFactory;
    }
    createProcess() {
        if (this.proc)
            return;
        ...
        try {
            if (typeof global.Worker === 'function') {
                ...
                this.proc.onmessage = (e) => this.handleMessage(e.data);
                this.proc.postMessage = (m) => this.proc.postMessage(m);
                return;
            }
        }
        catch (e) {
        ...
        try {
            ...
            if (typeof instance.onmessage === 'undefined') {
                ...
                instance.onmessage = (data) => this.handleMessage(data);
            }
            else {
                // Node style uses postMessage and 'message' events via callbacks
                instance.onmessage = (ev) => this.handleMessage(ev.data || ev);
            }
        }
        catch (err) {
    ...
    async calculate(fen, depth = 10, movetime, options) {
        this.start();
        const infos = [];
