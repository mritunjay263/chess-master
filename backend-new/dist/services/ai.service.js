...
const logger = (0, logger_1.createLogger)('ai-service');
...
class AIService {
     ...
     */
    async getBestMove(fen, depth = 3, skillLevel = 5) {
        ...
        try {
            ...
            logger.error('Error getting best move', { error: error.message });
            return null;
        }
    }
    /**
     ...
     */
    async getMoveSuggestions(fen, count = 3) {
        const chess = new chess_js_1.Chess(fen);
        if (chess.isGameOver()) {
            return [];
        }
        try {
            ...
            logger.error('Error getting move suggestions', { error: error.message });
            return [];
        }
    }
    generateAllMoves(chess) {
